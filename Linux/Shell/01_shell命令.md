...
| 选项 | 说明 | 示例 |
|------|------|------|
| `-r` | 降序排列 | `sort -r names.txt` |
| `-n` | 按数值排序 | `sort -n numbers.txt` |
| `-k N` | 按第 N 列排序 | `sort -k2 data.txt` |
| `-t SEP` | 指定列分隔符 | `sort -t: -k3 /etc/passwd` |
| `-u` | 去重（只保留唯一行） | `sort -u items.txt` |
| `-h` | 人类可读数值（K/M/G） | `sort -h sizes.txt` |
| `-o FILE` | 输出到文件（可用于原地排序） | `sort -o sorted.txt input.txt` |
| `-R` | 随机排序 | `sort -R list.txt` |

### 典型用法

```bash
# 变量内容排序（配合 comm 做集合运算）
REMOTE=$(get_skill_names "$DIR1" | sort)
LOCAL=$(get_skill_names "$DIR2" | sort)

# 按数值排序（不指定 -n 则按字典序，"10" < "2"）
echo -e "3\n12\n1" | sort -n
# 1
# 3
# 12
...
# 对齐排序结果写入原文件（-o 可安全原地写入）
sort -o file.txt file.txt
```

### 要点

- 默认字典序：`2` > `10`（按字符逐位比较），数值排序需加 `-n`
- `sort` 是 `comm` 的前置依赖——两个输入必须先排序才能正确比较
- `-o` 输出到源文件本身是安全的（GNU sort 内部做了处理）

---

## comm — 逐行比较两个已排序文件

逐行比较两个**已排序**的文件，输出三列：仅文件1、仅文件2、共同行。
...
| 组合 | 说明 | 示例 |
|------|------|------|
| `-23` | 仅文件 1 独有 | `comm -23 a b` |
| `-13` | 仅文件 2 独有 | `comm -13 a b` |
| `-12` | 两文件共有的行 | `comm -12 a b` |
| 无选项 | 显示全部三列 | `comm a b` |
